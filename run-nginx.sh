...
nginx_write_vhost() {
    ...
    # from Linux). Self-healing: the corrected config is written back too. Does
    # not apply to localhost/127.0.0.1 — they share $WEB_ROOT as docroot, so
    # they'd collide on the same project_conf path instead of each getting their own.
